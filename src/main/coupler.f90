...
!    !$omp parallel do collapse(2) private(i,j,ip1,jp1)
 ...
 !   !$omp end parallel do
  ...
  subroutine geo_to_smb(geo,smb)
    ...
    type(map_class) :: maps_geo_to_smb
    ...
    ! map ice thickness
    call map_field(maps_geo_to_smb,"h_ice",geo%hires%h_ice,smb%h_ice,method="mean",missing_value=-9999._dp)
    ...
    elsewhere
      mask_ice_geo = 0.
    endwhere
    call map_field(maps_geo_to_smb,"mask",mask_ice_geo,mask_ice,method="mean",missing_value=-9999._dp)
    where (mask_ice<0.5) smb%h_ice = 0._wp
    deallocate(mask_ice)
    deallocate(mask_ice_geo)
      
    ! standard deviation of bedrock topography
    call map_field(maps_geo_to_smb,"z_bed_std",geo%hires%z_bed_std,smb%z_bed_std,method="mean",missing_value=-9999._dp)
  ...
  subroutine geo_to_bmb(geo,bmb)
    ...
    type(map_class) :: maps_geo_to_bmb
    ...
    call map_field(maps_geo_to_bmb,"h_ice",geo%hires%h_ice,h_ice,method="mean",missing_value=-9999._dp)
    ...
    elsewhere
      mask_ice_geo = 0.
    endwhere
    call map_field(maps_geo_to_bmb,"mask",mask_ice_geo,mask_ice,method="mean",missing_value=-9999._dp)
    where (mask_ice<0.5) h_ice = 0._wp
    deallocate(mask_ice)
    deallocate(mask_ice_geo)
    call map_field(maps_geo_to_bmb,"z_bed",geo%hires%z_bed,z_bed,method="mean",missing_value=-9999._dp)
  ...
  subroutine ice_to_geo(n_ice_domain,ice,geo)
    ...
    type(map_class), save, allocatable, dimension(:) :: maps_hice_to_geo
    type(map_class), save, allocatable, dimension(:) :: maps_mask_to_geo
    ...
    do n=1,n_ice_domain
      ! ice thickness
      call map_field(maps_hice_to_geo(n),"hice",ice(n)%H_ice,geo%hires%h_ice,method="mean",missing_value=-9999._dp,reset=.false.)
      ...
      endwhere
      mask_ice_geo = 1.
      call map_field(maps_hice_to_geo(n),"mask",mask_ice,mask_ice_geo,method="mean",missing_value=-9999._dp,reset=.false.)
      where (mask_ice_geo<0.5) geo%hires%h_ice = 0._wp
      !mask_ice_geo = 0._wp
      !call map_field(maps_hice_to_geo(n),"mask",mask_ice,mask_ice_geo,method="mean",missing_value=-9999._wp,reset=.false.)
...
!    endwhere
  ...
  subroutine geo_to_ice(n_ice_domain,geo,ice)
    ...
    integer :: n 
    type(map_class), save, allocatable, dimension(:) :: maps_geo_to_ice
    type(map_class), save, allocatable, dimension(:) :: maps_geo_to_ice_nn
    ...
    do n=1,n_ice_domain
      ...
      call map_field(maps_geo_to_ice_nn(n),"mask_ocn_lake",geo%hires%mask_ocn_lake,ice(n)%mask_ocn_lake,method="mean",missing_value=-9999)
      !$omp section
      ! map sea (lake) level to ice grid, sea level over ocean is ==0
      call map_field(maps_geo_to_ice(n),"z_sl",geo%hires%z_sea_lake,ice(n)%z_sl,method="mean",missing_value=-9999._dp, &
        filt_method="gaussian",filt_par=[100._dp,ice(n)%grid%G%dx])
      !$omp section
      ! map lithosphere elevation relative to contemporary sea level (geoid)
      call map_field(maps_geo_to_ice(n),"zbed",geo%hires%z_bed,ice(n)%z_bed,method="mean",missing_value=-9999._dp)
      !$omp section
      ! map lithosphere elevation relative to contemporary sea level (geoid), filtered
      call map_field(maps_geo_to_ice(n),"zbed",geo%hires%z_bed,ice(n)%z_bed_fil,method="mean",missing_value=-9999._dp, &
        filt_method="gaussian",filt_par=[100._dp,ice(n)%grid%G%dx])
      !$omp section
      ! map standard deviation of sub-grid bedrock topography 
      call map_field(maps_geo_to_ice(n),"zbedstd",geo%hires%z_bed_std,ice(n)%z_bed_std,method="mean",missing_value=-9999._dp, &
        filt_method="gaussian",filt_par=[50._dp,ice(n)%grid%G%dx])
      !$omp section
      ! map geothermal heat flux to ice grid
      call map_field(maps_geo_to_ice(n),"q_geo",geo%hires%q_geo_ice,ice(n)%q_geo,method="mean",missing_value=-9999._dp, &
        filt_method="gaussian",filt_par=[100._dp,ice(n)%grid%G%dx])
      !$omp section
      ! map sediment mask to ice grid
      call map_field(maps_geo_to_ice(n),"h_sed",geo%hires%h_sed,ice(n)%H_sed,method="mean",missing_value=-9999._dp)
      !$omp end parallel sections
  ...
  subroutine bnd_to_geo(bnd,geo)
    ...
    real(wp), allocatable, dimension(:,:) :: mask_ice
    real(wp), allocatable, dimension(:,:) :: mask_ice_geo
    type(map_class), save :: maps_bndice_to_geo
    type(map_class), save :: maps_bndgeo_to_geo
    ...
    if (.not.flag_ice .or. ifake_ice.eq.1) then
      if (l_samegrid_geo_bndice) then
        ...
        call map_field(maps_bndice_to_geo,"hice",bnd%ice%h_ice,geo%hires%h_ice,method="mean",missing_value=-9999._dp,reset=.false.)
    ...
    if (.not.flag_geo) then
      if (l_samegrid_geo_bndgeo) then
        ...
        call map_field(maps_bndgeo_to_geo,"zbed",bnd%geo%z_bed-bnd%geo%z_bed_ref,z_bed_anom,method="mean",missing_value=-9999._dp)
