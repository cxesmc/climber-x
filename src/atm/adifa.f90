...
contains
  ...
  subroutine adifa(fax, fay, tp, q3, qam, d3, cam, diffxdse, diffydse, diffxwtr, diffywtr, diffxdst, diffydst, &
    ...
    real(wp) :: r16_ij, r18_ij, r16_i1j, r18_i1j, r16_ij1, r18_ij1, rup16, rup18


    !$omp parallel do private(i, j, k, imi, jmi, tpup, qup, dup, cup, dpl_x, dpl_y, rup16, rup18) &
    !$omp private (tp_ijk, tp_i1jk, tp_ij1k, q3_ijk, q3_i1jk, q3_ij1k, d3_ijk, d3_i1jk, d3_ij1k, c3_ij, c3_i1j, c3_ij1, fax_ijk, fay_ijk, r16_ij, r18_ij, r16_i1j, r18_i1j, r16_ij1, r18_ij1)
    do j=1,jm
      ...
      do i=1,im
        ...
        ! near-surface oxygen-isotope ratios (isotope-water/total water) of this cell and its
        ! upstream neighbours, used to transport the isotopes upwind-consistently with bulk water
        r16_ij  = qam(i  ,j  ,2)/qam(i  ,j  ,1)
        r18_ij  = qam(i  ,j  ,3)/qam(i  ,j  ,1)
        r16_i1j = qam(imi,j  ,2)/qam(imi,j  ,1)
        r18_i1j = qam(imi,j  ,3)/qam(imi,j  ,1)
        r16_ij1 = qam(i  ,jmi,2)/qam(i  ,jmi,1)
        r18_ij1 = qam(i  ,jmi,3)/qam(i  ,jmi,1)
        ...
        do k=1,km
          ...
          faxwtr(i,j,2) = faxwtr(i,j,2) + fax_ijk*qup*rup16  
          faxwtr(i,j,3) = faxwtr(i,j,3) + fax_ijk*qup*rup18
          ...
          faywtr(i,j,2) = faywtr(i,j,2) + fay_ijk*qup*rup16  
          faywtr(i,j,3) = faywtr(i,j,3) + fay_ijk*qup*rup18
          ...
          if (k.le.km-2) then   ! limit to troposphere
            ...
            fdxwtr(i,j,2) = fdxwtr(i,j,2) + diffxwtr(i,j)*dy*dpl_x*(q3_i1jk*r16_i1j-q3_ijk*r16_ij)/dxt(j)
            fdxwtr(i,j,3) = fdxwtr(i,j,3) + diffxwtr(i,j)*dy*dpl_x*(q3_i1jk*r18_i1j-q3_ijk*r18_ij)/dxt(j)
            ...
            fdywtr(i,j,2) = fdywtr(i,j,2) + diffywtr(i,j)*dxu(j)*dpl_y*(q3_ijk*r16_ij-q3_ij1k*r16_ij1)/dy
            fdywtr(i,j,3) = fdywtr(i,j,3) + diffywtr(i,j)*dxu(j)*dpl_y*(q3_ijk*r18_ij-q3_ij1k*r18_ij1)/dy
