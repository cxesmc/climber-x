...
module ocn_out
  ...
  use dim_name, only: len, dim_lon, dim_lonu, dim_lat, dim_latv, dim_latv1, dim_lev, dim_levw, dim_time, dim_month, dim_day, dim_type, dim_isles
  ...
  integer :: j_ibe, i_ibe
  real(wp), parameter :: lat_pacN = 0._wp     !! southern boundary of the North Pacific region [degN]
  real(wp), parameter :: lat_pacN30 = 30._wp  !! southern boundary of the North Pacific >30N region [degN]
  ...
  type ts_out
     ...
     real(wp) :: mld_atlN50, mld_lab, mld_irm, mld_gin, mld_bkn, mld_wedd, mld_ross, mld_so, mld_pacN, mld_pacN30
     real(wp) :: mldst_atlN50, mldst_lab, mldst_irm, mldst_gin, mldst_bkn, mldst_wedd, mldst_ross, mldst_so, mldst_pacN, mldst_pacN30
     real(wp) :: pe_atlN, pe_atlN50, pe_lab, pe_irm, pe_gin, pe_bkn, pe_wedd, pe_ross, pe_so, pe_pacN, pe_pacN30
     real(wp) :: buoy_lab, buoy_irm, buoy_gin, buoy_bkn, buoy_wedd, buoy_ross, buoy_so, buoy_soS60, buoyT_soS60, buoyS_soS60, buoy_pacN, buoy_pacN30
     real(wp) :: sst_atlN50, sst_lab, sst_irm, sst_gin, sst_bkn, sst_wedd, sst_ross, sst_so, sst_pacN, sst_pacN30
     real(wp) :: sss_atlN50, sss_lab, sss_irm, sss_gin, sss_bkn, sss_wedd, sss_ross, sss_so, sss_pacN, sss_pacN30
     real(wp) :: sst_ibe
  end type
...
!        print *,''
    ...
    do i=i_atlN50(1),i_atlN50(2)
      do j=j_atlN50(1),j_atlN50(2)
        if (ocn%f_ocn(i,j).gt.0._wp) then
          ...
          sst_atlN50 = sst_atlN50 + ocn%ts(i,j,maxk,1)*ocn%grid%ocn_area(i,j)
          sss_atlN50 = sss_atlN50 + ocn%ts(i,j,maxk,2)*ocn%grid%ocn_area(i,j)
          area_atlN50 = area_atlN50 + ocn%grid%ocn_area(i,j)
        endif
      enddo
    enddo
    sst_atlN50 = sst_atlN50/area_atlN50
    sss_atlN50 = sss_atlN50/area_atlN50
    ...
    do i=i_lab(1),i_lab(2)
      do j=j_lab(1),j_lab(2)
        if (ocn%f_ocn(i,j).gt.0._wp) then
          ...
          sst_lab = sst_lab + ocn%ts(i,j,maxk,1)*ocn%grid%ocn_area(i,j)
          sss_lab = sss_lab + ocn%ts(i,j,maxk,2)*ocn%grid%ocn_area(i,j)
          area_lab = area_lab + ocn%grid%ocn_area(i,j)
        endif
      enddo
    enddo
    sst_lab = sst_lab/area_lab
    sss_lab = sss_lab/area_lab
    ...
    do i=i_irm(1),i_irm(2)
      do j=j_irm(1),j_irm(2)
        if (ocn%f_ocn(i,j).gt.0._wp) then
          ...
          sst_irm = sst_irm + ocn%ts(i,j,maxk,1)*ocn%grid%ocn_area(i,j)
          sss_irm = sss_irm + ocn%ts(i,j,maxk,2)*ocn%grid%ocn_area(i,j)
          area_irm = area_irm + ocn%grid%ocn_area(i,j)
        endif
      enddo
    enddo
    sst_irm = sst_irm/area_irm
    sss_irm = sss_irm/area_irm
    ...
    do i=i_gin(1),i_gin(2)
      do j=j_gin(1),j_gin(2)
        if (ocn%f_ocn(i,j).gt.0._wp) then
          ...
          sst_gin = sst_gin + ocn%ts(i,j,maxk,1)*ocn%grid%ocn_area(i,j)
          sss_gin = sss_gin + ocn%ts(i,j,maxk,2)*ocn%grid%ocn_area(i,j)
          area_gin = area_gin + ocn%grid%ocn_area(i,j)
        endif
      enddo
    enddo
    sst_gin = sst_gin/area_gin
    sss_gin = sss_gin/area_gin
    ...
    do i=i_bkn(1),i_bkn(2)
      do j=j_bkn(1),j_bkn(2)
        if (ocn%f_ocn(i,j).gt.0._wp) then
          ...
          sst_bkn = sst_bkn + ocn%ts(i,j,maxk,1)*ocn%grid%ocn_area(i,j)
          sss_bkn = sss_bkn + ocn%ts(i,j,maxk,2)*ocn%grid%ocn_area(i,j)
          area_bkn = area_bkn + ocn%grid%ocn_area(i,j)
        endif
      enddo
    enddo
    sst_bkn = sst_bkn/area_bkn
    sss_bkn = sss_bkn/area_bkn
    ...
    do i=i_wedd(1),i_wedd(2)
      do j=j_wedd(1),j_wedd(2)
        if (ocn%f_ocn(i,j).gt.0._wp) then
          ...
          sst_wedd = sst_wedd + ocn%ts(i,j,maxk,1)*ocn%grid%ocn_area(i,j)
          sss_wedd = sss_wedd + ocn%ts(i,j,maxk,2)*ocn%grid%ocn_area(i,j)
          area_wedd = area_wedd + ocn%grid%ocn_area(i,j)
        endif
      enddo
    enddo
    sst_wedd = sst_wedd/area_wedd
    sss_wedd = sss_wedd/area_wedd
    ...
    do i=i_ross(1),i_ross(2)
      do j=j_ross(1),j_ross(2)
        if (ocn%f_ocn(i,j).gt.0._wp) then
          ...
          sst_ross = sst_ross + ocn%ts(i,j,maxk,1)*ocn%grid%ocn_area(i,j)
          sss_ross = sss_ross + ocn%ts(i,j,maxk,2)*ocn%grid%ocn_area(i,j)
          area_ross = area_ross + ocn%grid%ocn_area(i,j)
        endif
      enddo
    enddo
    sst_ross = sst_ross/area_ross
    sss_ross = sss_ross/area_ross
    ...
    do i=i_so(1),i_so(2)
      do j=j_so(1),j_so(2)
        if (ocn%f_ocn(i,j).gt.0._wp) then
          ...
          sst_so = sst_so + ocn%ts(i,j,maxk,1)*ocn%grid%ocn_area(i,j)
          sss_so = sss_so + ocn%ts(i,j,maxk,2)*ocn%grid%ocn_area(i,j)
          area_so = area_so + ocn%grid%ocn_area(i,j)
        endif
      enddo
    enddo
    sst_so = sst_so/area_so
    sss_so = sss_so/area_so
    ...
    do i=1,maxi
      do j=1,maxj
        if (ocn%f_ocn(i,j).gt.0._wp .and. basin_mask(i,j).eq.i_pacific .and. lat(j).ge.lat_pacN) then
          mldst = -zw(k1(i,j))
          rho_maxk = eos(ocn%ts(i,j,maxk,1),ocn%ts(i,j,maxk,2),0._wp)    ! in-situ density at the surface
          do k=maxk,k1(i,j),-1
            rho_k = eos(ocn%ts(i,j,k,1),ocn%ts(i,j,k,2),0._wp)   ! in-situ density
          ...
          enddo
          mldst_pacN = max(mldst_pacN,mldst)
          mld_pacN = max(mld_pacN,-ocn%mld(i,j))
          pe_pacN = pe_pacN + ocn%conv_pe(i,j)*ocn%grid%ocn_area(i,j)   ! J/m2*m2=J
          alpha = alpha2d(i,j)
          beta = 0.8_wp   ! kg/m3/psu
          buoy_pacN = buoy_pacN + (ocn%flx(i,j)*alpha/cap_w*g/rho0 + ocn%fw_corr(i,j)*beta*ocn%saln0*g/rho_w)*ocn%grid%ocn_area(i,j)*dt  ! N 
          sst_pacN = sst_pacN + ocn%ts(i,j,maxk,1)*ocn%grid%ocn_area(i,j)
          sss_pacN = sss_pacN + ocn%ts(i,j,maxk,2)*ocn%grid%ocn_area(i,j)
          area_pacN = area_pacN + ocn%grid%ocn_area(i,j)
        endif
      enddo
    enddo
    sst_pacN = sst_pacN/area_pacN
    sss_pacN = sss_pacN/area_pacN
    ...
    do i=1,maxi
      do j=1,maxj
        if (ocn%f_ocn(i,j).gt.0._wp .and. basin_mask(i,j).eq.i_pacific .and. lat(j).ge.lat_pacN30) then
          mldst = -zw(k1(i,j))
          rho_maxk = eos(ocn%ts(i,j,maxk,1),ocn%ts(i,j,maxk,2),0._wp)    ! in-situ density at the surface
          do k=maxk,k1(i,j),-1
            rho_k = eos(ocn%ts(i,j,k,1),ocn%ts(i,j,k,2),0._wp)   ! in-situ density
          ...
          enddo
          mldst_pacN30 = max(mldst_pacN30,mldst)
          mld_pacN30 = max(mld_pacN30,-ocn%mld(i,j))
          pe_pacN30 = pe_pacN30 + ocn%conv_pe(i,j)*ocn%grid%ocn_area(i,j)   ! J/m2*m2=J
          alpha = alpha2d(i,j)
          beta = 0.8_wp   ! kg/m3/psu
          buoy_pacN30 = buoy_pacN30 + (ocn%flx(i,j)*alpha/cap_w*g/rho0 + ocn%fw_corr(i,j)*beta*ocn%saln0*g/rho_w)*ocn%grid%ocn_area(i,j)*dt  ! N 
          sst_pacN30 = sst_pacN30 + ocn%ts(i,j,maxk,1)*ocn%grid%ocn_area(i,j)
          sss_pacN30 = sss_pacN30 + ocn%ts(i,j,maxk,2)*ocn%grid%ocn_area(i,j)
          area_pacN30 = area_pacN30 + ocn%grid%ocn_area(i,j)
        endif
      enddo
    enddo
    sst_pacN30 = sst_pacN30/area_pacN30
    sss_pacN30 = sss_pacN30/area_pacN30
    ...
    if( time_soy_ocn ) then
      ...
      ann_ts(y)%mld_pacN = 0._wp
      ann_ts(y)%mld_pacN30 = 0._wp
      ...
      ann_ts(y)%mldst_pacN = 0._wp
      ann_ts(y)%mldst_pacN30 = 0._wp
      ...
      ann_ts(y)%pe_pacN = 0._wp
      ann_ts(y)%pe_pacN30 = 0._wp
      ...
      ann_ts(y)%buoy_pacN = 0._wp
      ann_ts(y)%buoy_pacN30 = 0._wp
      ann_ts(y)%sst_atlN50 = 0._wp
      ann_ts(y)%sst_lab = 0._wp
      ann_ts(y)%sst_irm = 0._wp
      ann_ts(y)%sst_gin = 0._wp
      ann_ts(y)%sst_bkn = 0._wp
      ann_ts(y)%sst_wedd = 0._wp
      ann_ts(y)%sst_ross = 0._wp
      ann_ts(y)%sst_so = 0._wp
      ann_ts(y)%sst_pacN = 0._wp
      ann_ts(y)%sst_pacN30 = 0._wp
      ann_ts(y)%sst_ibe = 0._wp
      ann_ts(y)%sss_atlN50 = 0._wp
      ann_ts(y)%sss_lab = 0._wp
      ann_ts(y)%sss_irm = 0._wp
      ann_ts(y)%sss_gin = 0._wp
      ann_ts(y)%sss_bkn = 0._wp
      ann_ts(y)%sss_wedd = 0._wp
      ann_ts(y)%sss_ross = 0._wp
      ann_ts(y)%sss_so = 0._wp
      ann_ts(y)%sss_pacN = 0._wp
      ann_ts(y)%sss_pacN30 = 0._wp
    ...
    ann_ts(y)%mld_pacN = max(ann_ts(y)%mld_pacN,mld_pacN)
    ann_ts(y)%mld_pacN30 = max(ann_ts(y)%mld_pacN30,mld_pacN30)
    ...
    ann_ts(y)%mldst_pacN = max(ann_ts(y)%mldst_pacN,mldst_pacN)
    ann_ts(y)%mldst_pacN30 = max(ann_ts(y)%mldst_pacN30,mldst_pacN30)
    ...
    ann_ts(y)%pe_pacN = ann_ts(y)%pe_pacN + pe_pacN
    ann_ts(y)%pe_pacN30 = ann_ts(y)%pe_pacN30 + pe_pacN30
    ...
    ann_ts(y)%buoy_pacN = ann_ts(y)%buoy_pacN + buoy_pacN
    ann_ts(y)%buoy_pacN30 = ann_ts(y)%buoy_pacN30 + buoy_pacN30
    ann_ts(y)%sst_atlN50   = ann_ts(y)%sst_atlN50   + sst_atlN50   * ann_avg 
    ann_ts(y)%sst_lab   = ann_ts(y)%sst_lab   + sst_lab   * ann_avg 
    ann_ts(y)%sst_irm   = ann_ts(y)%sst_irm   + sst_irm   * ann_avg 
    ann_ts(y)%sst_gin  = ann_ts(y)%sst_gin  + sst_gin  * ann_avg
    ann_ts(y)%sst_bkn  = ann_ts(y)%sst_bkn  + sst_bkn  * ann_avg
    ann_ts(y)%sst_wedd = ann_ts(y)%sst_wedd + sst_wedd * ann_avg
    ann_ts(y)%sst_ross = ann_ts(y)%sst_ross + sst_ross * ann_avg
    ann_ts(y)%sst_so   = ann_ts(y)%sst_so   + sst_so  * ann_avg
    ann_ts(y)%sst_ibe  = ann_ts(y)%sst_ibe  + 0.5_wp*(ocn%ts(i_ibe,j_ibe,maxk,1)+ocn%ts(i_ibe-1,j_ibe,maxk,1))  * ann_avg
    ann_ts(y)%sst_pacN = ann_ts(y)%sst_pacN + sst_pacN * ann_avg
    ann_ts(y)%sst_pacN30 = ann_ts(y)%sst_pacN30 + sst_pacN30 * ann_avg
    ann_ts(y)%sss_atlN50   = ann_ts(y)%sss_atlN50   + sss_atlN50   * ann_avg 
    ann_ts(y)%sss_lab   = ann_ts(y)%sss_lab   + sss_lab   * ann_avg 
    ann_ts(y)%sss_irm   = ann_ts(y)%sss_irm   + sss_irm   * ann_avg 
    ann_ts(y)%sss_gin  = ann_ts(y)%sss_gin  + sss_gin  * ann_avg
    ann_ts(y)%sss_bkn  = ann_ts(y)%sss_bkn  + sss_bkn  * ann_avg
    ann_ts(y)%sss_wedd = ann_ts(y)%sss_wedd + sss_wedd * ann_avg
    ann_ts(y)%sss_ross = ann_ts(y)%sss_ross + sss_ross * ann_avg
    ann_ts(y)%sss_so   = ann_ts(y)%sss_so   + sss_so  * ann_avg
    ann_ts(y)%sss_pacN = ann_ts(y)%sss_pacN + sss_pacN * ann_avg
    ann_ts(y)%sss_pacN30 = ann_ts(y)%sss_pacN30 + sss_pacN30 * ann_avg
    ...
    if( time_eoy_ocn ) then
       ...
       if (mod(year,10).eq.1) then
         print '(a7,a9,17a7)','ocn','year','SST','SSS','TVOL','SVOL','AMOC','PMOC','AABW','hAMOC','FWFnet','FWAtl','BAtl55','Qnets','Qnetb','Drake','Indo','Bering','Ashelf'
       endif

       print '(a7,i9,8F7.1,2F7.2,1F7.1,2F7.2,4F7.1)', &
         'ocn',year_now,ann_ts(y)%sst,ann_ts(y)%sss,ann_ts(y)%t,ann_ts(y)%s,ann_ts(y)%omaxa,ann_ts(y)%omaxp,-ann_ts(y)%omins, &
         ann_ts(y)%hmaxa,ann_ts(y)%fw(1),ann_ts(y)%fw(2),ann_ts(y)%buoy_NA(ilatv_buoy_sel)*1.e-15,ann_ts(y)%flx(1)*1.e15_wp/ocn%grid%ocn_area_tot, & 
         sum(ocn%q_geo*ocn_area)/ocn%grid%ocn_area_tot, &
         ann_ts(y)%drake,ann_ts(y)%indo,ann_ts(y)%bering,ann_ts(y)%shelf
  ...
  dim1=dim_time,start=[ndat],count=[y],ncid=ncid)    
    ...
    call nc_write(fnm,"mld_pacN",   vars%mld_pacN,dim1=dim_time,start=[ndat],count=[y],long_name="Maximum mixed layer depth in the North Pacific",units="m",ncid=ncid)
    call nc_write(fnm,"mld_pacN30",   vars%mld_pacN30,dim1=dim_time,start=[ndat],count=[y],long_name="Maximum mixed layer depth in the North Pacific (>30N)",units="m",ncid=ncid)
    ...
    call nc_write(fnm,"mldst_pacN",   vars%mldst_pacN,dim1=dim_time,start=[ndat],count=[y],long_name="Maximum mixed layer depth based on sigma-t in the North Pacific",units="m",ncid=ncid)
    call nc_write(fnm,"mldst_pacN30",   vars%mldst_pacN30,dim1=dim_time,start=[ndat],count=[y],long_name="Maximum mixed layer depth based on sigma-t in the North Pacific (>30N)",units="m",ncid=ncid)
    ...
    call nc_write(fnm,"pe_pacN",   vars%pe_pacN,dim1=dim_time,start=[ndat],count=[y],long_name="Potential energy released by convection in the North Pacific",units="J",ncid=ncid)
    call nc_write(fnm,"pe_pacN30",   vars%pe_pacN30,dim1=dim_time,start=[ndat],count=[y],long_name="Potential energy released by convection in the North Pacific (>30N)",units="J",ncid=ncid)
    ...
    call nc_write(fnm,"buoy_pacN",   vars%buoy_pacN,dim1=dim_time,start=[ndat],count=[y],long_name="Buoyancy flux over the North Pacific",units="N",ncid=ncid)
    call nc_write(fnm,"buoy_pacN30",   vars%buoy_pacN30,dim1=dim_time,start=[ndat],count=[y],long_name="Buoyancy flux over the North Pacific (>30N)",units="N",ncid=ncid)

    call nc_write(fnm,"sst_atlN50",   vars%sst_atlN50,dim1=dim_time,start=[ndat],count=[y],  long_name="Average sea surface temperature in Atlantic >50N",units="degC",ncid=ncid)
    call nc_write(fnm,"sst_lab",   vars%sst_lab,dim1=dim_time,start=[ndat],count=[y],  long_name="Average sea surface temperature in Labrador Sea",units="degC",ncid=ncid)
    call nc_write(fnm,"sst_irm",   vars%sst_irm,dim1=dim_time,start=[ndat],count=[y],  long_name="Average sea surface temperature in Irminger Sea",units="degC",ncid=ncid)
    call nc_write(fnm,"sst_gin",  vars%sst_gin,dim1=dim_time,start=[ndat],count=[y], long_name="Average sea surface temperature in the GIN seas",units="degC",ncid=ncid)
    call nc_write(fnm,"sst_bkn",  vars%sst_bkn,dim1=dim_time,start=[ndat],count=[y], long_name="Average sea surface temperature in the Barents-Kara-Nansen seas",units="degC",ncid=ncid)
    call nc_write(fnm,"sst_wedd", vars%sst_wedd,dim1=dim_time,start=[ndat],count=[y],long_name="Average sea surface temperature in the Weddel Sea",units="degC",ncid=ncid)
    call nc_write(fnm,"sst_ross", vars%sst_ross,dim1=dim_time,start=[ndat],count=[y],long_name="Average sea surface temperature in the Ross Sea",units="degC",ncid=ncid)
    call nc_write(fnm,"sst_so",   vars%sst_so,dim1=dim_time,start=[ndat],count=[y],  long_name="Average sea surface temperature in the Southern Ocean around Antarctica",units="degC",ncid=ncid)
    call nc_write(fnm,"sst_ibe",  vars%sst_ibe,dim1=dim_time,start=[ndat],count=[y], long_name="Annual mean sea surface temperature at the Iberian margin",units="degC",ncid=ncid)
    call nc_write(fnm,"sst_pacN",   vars%sst_pacN,dim1=dim_time,start=[ndat],count=[y],long_name="Average sea surface temperature in the North Pacific",units="degC",ncid=ncid)
    call nc_write(fnm,"sst_pacN30",   vars%sst_pacN30,dim1=dim_time,start=[ndat],count=[y],long_name="Average sea surface temperature in the North Pacific (>30N)",units="degC",ncid=ncid)

    call nc_write(fnm,"sss_atlN50",   vars%sss_atlN50,dim1=dim_time,start=[ndat],count=[y],  long_name="Average sea surface salinity in Atlantic >50N",units="psu",ncid=ncid)
    call nc_write(fnm,"sss_lab",   vars%sss_lab,dim1=dim_time,start=[ndat],count=[y],  long_name="Average sea surface salinity in Labrador Sea",units="psu",ncid=ncid)
    call nc_write(fnm,"sss_irm",   vars%sss_irm,dim1=dim_time,start=[ndat],count=[y],  long_name="Average sea surface salinity in Irminger Sea",units="psu",ncid=ncid)
    call nc_write(fnm,"sss_gin",  vars%sss_gin,dim1=dim_time,start=[ndat],count=[y], long_name="Average sea surface salinity in the GIN seas",units="psu",ncid=ncid)
    call nc_write(fnm,"sss_bkn",  vars%sss_bkn,dim1=dim_time,start=[ndat],count=[y], long_name="Average sea surface salinity in the Barents-Kara-Nansen seas",units="psu",ncid=ncid)
    call nc_write(fnm,"sss_wedd", vars%sss_wedd,dim1=dim_time,start=[ndat],count=[y],long_name="Average sea surface salinity in the Weddel Sea",units="psu",ncid=ncid)
    call nc_write(fnm,"sss_ross", vars%sss_ross,dim1=dim_time,start=[ndat],count=[y],long_name="Average sea surface salinity in the Ross Sea",units="psu",ncid=ncid)
    call nc_write(fnm,"sss_so",   vars%sss_so,dim1=dim_time,start=[ndat],count=[y],  long_name="Average sea surface salinity in the Southern Ocean around Antarctica",units="psu",ncid=ncid)
    call nc_write(fnm,"sss_pacN",   vars%sss_pacN,dim1=dim_time,start=[ndat],count=[y],long_name="Average sea surface salinity in the North Pacific",units="psu",ncid=ncid)
    call nc_write(fnm,"sss_pacN30",   vars%sss_pacN30,dim1=dim_time,start=[ndat],count=[y],long_name="Average sea surface salinity in the North Pacific (>30N)",units="psu",ncid=ncid)
  ...
  subroutine ocn_nc(fnm)
    ...
    call nc_write_dim(fnm,dim_latv1,x=latv(1:maxj+1),axis="y",ncid=ncid)
    ...
    call nc_write(fnm,"dz", dz(maxk:1:-1), dims=[dim_lev],start=[1],count=[maxk],long_name="thickness of grid cells",units="m",ncid=ncid)
    call nc_write(fnm,"dzw", dza(maxk:1:-1), dims=[dim_levw],start=[1],count=[maxk],long_name="thickness between layer centers",units="m",ncid=ncid)
    call nc_close(ncid)
  ...
  subroutine ocn_nc_write(fnm,ncid,vars,ndat,nout)
    ...
    call nc_write(fnm,"t",        sngl(vars%t),  dims=[dim_lon,dim_lat,dim_lev,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="potential temperature",units="C",missing_value=missing_value,ncid=ncid)
    ...
    call nc_write(fnm,"t_atl",    sngl(vars%t_atl),  dims=[dim_lat,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Zonal mean Atlantic potential temperature",units="C",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"t_pac",    sngl(vars%t_pac),  dims=[dim_lat,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Zonal mean Pacific potential temperature",units="C",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"t_ind",    sngl(vars%t_ind),  dims=[dim_lat,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Zonal mean Indian Ocean potential temperature",units="C",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"t_so",    sngl(vars%t_so),  dims=[dim_lat,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Zonal mean Southern Ocean potential temperature",units="C",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"s",        sngl(vars%s),  dims=[dim_lon,dim_lat,dim_lev,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="salinity",units="psu",missing_value=missing_value,ncid=ncid)
    ...
    call nc_write(fnm,"s_atl",    sngl(vars%s_atl),  dims=[dim_lat,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Zonal mean Atlantic salinity",units="psu",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"s_pac",    sngl(vars%s_pac),  dims=[dim_lat,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Zonal mean Pacific salinity",units="psu",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"s_ind",    sngl(vars%s_ind),  dims=[dim_lat,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Zonal mean Indian Ocean salinity",units="psu",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"s_so",    sngl(vars%s_so),  dims=[dim_lat,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Zonal mean Southern Ocean salinity",units="psu",missing_value=missing_value,ncid=ncid)
    if (age_tracer .and. ndat.eq.13) then
    call nc_write(fnm,"age",      sngl(vars%age),dims=[dim_lon,dim_lat,dim_lev,dim_time],start=[1,1,1,nout],count=[maxi,maxj,maxk,1],long_name="age",units="years",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"a_atl",    sngl(vars%a_atl),  dims=[dim_lat,dim_lev,dim_time],start=[1,1,nout],count=[maxj,maxk,1],long_name="Zonal mean Atlantic age",units="years",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"a_pac",    sngl(vars%a_pac),  dims=[dim_lat,dim_lev,dim_time],start=[1,1,nout],count=[maxj,maxk,1],long_name="Zonal mean Pacific age",units="years",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"a_ind",    sngl(vars%a_ind),  dims=[dim_lat,dim_lev,dim_time],start=[1,1,nout],count=[maxj,maxk,1],long_name="Zonal mean Indian Ocean age",units="years",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"a_so",     sngl(vars%a_so),   dims=[dim_lat,dim_lev,dim_time],start=[1,1,nout],count=[maxj,maxk,1],long_name="Zonal mean Southern Ocean age",units="years",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"dT_dt_adv", sngl(vars%dt_dt_adv),dims=[dim_lon,dim_lat,dim_lev,dim_time],start=[1,1,1,nout],count=[maxi,maxj,maxk,1],long_name="Temperature tendency due to advection",units="degC/year",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"dT_dt_diff", sngl(vars%dt_dt_diff),dims=[dim_lon,dim_lat,dim_lev,dim_time],start=[1,1,1,nout],count=[maxi,maxj,maxk,1],long_name="Temperature tendency due to diffusion",units="degC/year",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"dT_dt", sngl(vars%dt_dt_adv+vars%dt_dt_diff),dims=[dim_lon,dim_lat,dim_lev,dim_time],start=[1,1,1,nout],count=[maxi,maxj,maxk,1],long_name="Temperature tendency due to advection and diffusion",units="degC/year",missing_value=missing_value*2,ncid=ncid)
    ...
    call nc_write(fnm,"dS_dt_adv", sngl(vars%ds_dt_adv),dims=[dim_lon,dim_lat,dim_lev,dim_time],start=[1,1,1,nout],count=[maxi,maxj,maxk,1],long_name="Salinity tendency due to advection",units="psu/year",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"dS_dt_diff", sngl(vars%ds_dt_diff),dims=[dim_lon,dim_lat,dim_lev,dim_time],start=[1,1,1,nout],count=[maxi,maxj,maxk,1],long_name="Salinity tendency due to diffusion",units="psu/year",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"dS_dt", sngl(vars%ds_dt_adv+vars%ds_dt_diff),dims=[dim_lon,dim_lat,dim_lev,dim_time],start=[1,1,1,nout],count=[maxi,maxj,maxk,1],long_name="Salinity tendency due to advection and diffusion",units="psu/year",missing_value=missing_value*2,ncid=ncid)
    ...
    call nc_write(fnm,"dye",      sngl(vars%dye),dims=[dim_lon,dim_lat,dim_lev,dim_time],start=[1,1,1,nout],count=[maxi,maxj,maxk,1],long_name="dye tracer",units="/",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"d_atl",    sngl(vars%d_atl),  dims=[dim_lat,dim_lev,dim_time],start=[1,1,nout],count=[maxj,maxk,1],long_name="Zonal mean Atlantic dye",units="years",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"d_pac",    sngl(vars%d_pac),  dims=[dim_lat,dim_lev,dim_time],start=[1,1,nout],count=[maxj,maxk,1],long_name="Zonal mean Pacific dye",units="years",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"d_ind",    sngl(vars%d_ind),  dims=[dim_lat,dim_lev,dim_time],start=[1,1,nout],count=[maxj,maxk,1],long_name="Zonal mean Indian Ocean dye",units="years",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"d_so",     sngl(vars%d_so),   dims=[dim_lat,dim_lev,dim_time],start=[1,1,nout],count=[maxj,maxk,1],long_name="Zonal mean Southern Ocean dye",units="years",missing_value=missing_value,ncid=ncid)
    endif
    if (cons_tracer .and. ndat.eq.13) then
    call nc_write(fnm,"cons",      sngl(vars%cons),dims=[dim_lon,dim_lat,dim_lev,dim_time],start=[1,1,1,nout],count=[maxi,maxj,maxk,1],long_name="conservative tracer",units="/",missing_value=missing_value,ncid=ncid)
    endif
    if (l_cfc .and. ndat.eq.13) then
    call nc_write(fnm,"cfc11", sngl(vars%cfc11),dims=[dim_lon,dim_lat,dim_lev,dim_time],start=[1,1,1,nout],count=[maxi,maxj,maxk,1],long_name="CFC11",units="pmol/l",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"cfc12", sngl(vars%cfc12),dims=[dim_lon,dim_lat,dim_lev,dim_time],start=[1,1,1,nout],count=[maxi,maxj,maxk,1],long_name="CFC12",units="pmol/l",missing_value=missing_value,ncid=ncid)
    endif
    call nc_write(fnm,"rho",      sngl(vars%rho),dims=[dim_lon,dim_lat,dim_lev,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="in-situ density",units="kg/m3",missing_value=missing_value,ncid=ncid)
    ...
    call nc_write(fnm,"rho2",      sngl(vars%rho2),dims=[dim_lon,dim_lat,dim_lev,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="density",units="kg/m3",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"rho_atl",    sngl(vars%rho_atl),  dims=[dim_lat,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Zonal mean Atlantic in-situ density",units="kg/m3",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"rho_pac",    sngl(vars%rho_pac),  dims=[dim_lat,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Zonal mean Pacific in-situ density",units="kg/m3",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"rho_ind",    sngl(vars%rho_ind),  dims=[dim_lat,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Zonal mean Indian Ocean in-situ density",units="kg/m3",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"rho_so",    sngl(vars%rho_so),  dims=[dim_lat,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Zonal mean Southern Ocean in-situ density",units="kg/m3",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"taux",       sngl(vars%taux), dims=[dim_lonu,dim_lat,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxi,maxj,1,1],long_name="zonal wind/sea ice stress",units="N/m2",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"tauy",       sngl(vars%tauy), dims=[dim_lon,dim_latv,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxi,maxj,1,1],long_name="meridional wind/sea ice stress",units="N/m2",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"u",        sngl(vars%u),  dims=[dim_lonu,dim_lat,dim_lev,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="zonal velocity",units="m/s",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"v",        sngl(vars%v),  dims=[dim_lon,dim_latv,dim_lev,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="meridional velocity",units="m/s",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"w",        sngl(vars%w),  dims=[dim_lon,dim_lat,dim_levw,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="vertical velocity",units="m/s",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"ub",       sngl(vars%ub), dims=[dim_lonu,dim_lat,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxi,maxj,1,1],long_name="barotropic zonal velocity",units="m/s",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"vb",       sngl(vars%vb), dims=[dim_lon,dim_latv,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxi,maxj,1,1],long_name="barotropic meridional velocity",units="m/s",missing_value=missing_value,ncid=ncid)
    if (n_isles.gt.0) then
      call nc_write(fnm,"ubisl",       sngl(vars%ubisl), dims=[dim_lonu,dim_lat,dim_isles,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxisles,1,1],long_name="islands barotropic zonal velocity",units="m/s",missing_value=missing_value,ncid=ncid)
      call nc_write(fnm,"vbisl",       sngl(vars%vbisl), dims=[dim_lon,dim_latv,dim_isles,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxisles,1,1],long_name="islands barotropic meridional velocity",units="m/s",missing_value=missing_value,ncid=ncid)
    endif
    ...
    call nc_write(fnm,"hft",      sngl(vars%hft),dims=[dim_type,dim_latv,dim_month,dim_time],start=[1,1,ndat,nout],count=[5,maxj,1,1],long_name="Global ocean northward heat transport",units="PW",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"hfp",      sngl(vars%hfp),dims=[dim_type,dim_latv,dim_month,dim_time],start=[1,1,ndat,nout],count=[5,maxj,1,1],long_name="Indo-Pacific ocean northward heat transport",units="PW",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"hfa",      sngl(vars%hfa),dims=[dim_type,dim_latv,dim_month,dim_time],start=[1,1,ndat,nout],count=[5,maxj,1,1],long_name="Atlantic ocean northward heat transport",units="PW",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"hftz", sngl(vars%hftz),dims=[dim_latv,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Global ocean northward heat transport",units="PW/m",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"hfpz", sngl(vars%hfpz),dims=[dim_latv,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Indo-Pacific ocean northward heat transport",units="PW/m",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"hfaz", sngl(vars%hfaz),dims=[dim_latv,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Atlantic ocean northward heat transport",units="PW/m",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"fwt",      sngl(vars%fwt),dims=[dim_type,dim_latv,dim_month,dim_time],start=[1,1,ndat,nout],count=[5,maxj,1,1],long_name="Global ocean northward freshwater transport",units="Sv",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"fwp",      sngl(vars%fwp),dims=[dim_type,dim_latv,dim_month,dim_time],start=[1,1,ndat,nout],count=[5,maxj,1,1],long_name="Indo-Pacific ocean northward freshwater transport",units="Sv",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"fwa",      sngl(vars%fwa),dims=[dim_type,dim_latv,dim_month,dim_time],start=[1,1,ndat,nout],count=[5,maxj,1,1],long_name="Atlantic ocean northward freshwater transport",units="Sv",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"fwtz", sngl(vars%fwtz),dims=[dim_latv,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Global ocean northward freshwater transport",units="Sv/m",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"fwpz", sngl(vars%fwpz),dims=[dim_latv,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Indo-Pacific ocean northward freshwater transport",units="Sv/m",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"fwaz", sngl(vars%fwaz),dims=[dim_latv,dim_lev,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxj,maxk,1,1],long_name="Atlantic ocean northward freshwater transport",units="Sv/m",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"fayti",    sngl(vars%fayti),dims=[dim_lon,dim_latv,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxi,maxj,1,1],long_name="vertically integrated northward advective heat transport",units="W",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"fdyti",    sngl(vars%fdyti),dims=[dim_lon,dim_latv,dim_month,dim_time],start=[1,1,ndat,nout],count=[maxi,maxj,1,1],long_name="vertically integrated northward diffusive heat transport",units="W",missing_value=missing_value,ncid=ncid)
    ...
    call nc_write(fnm,"fdy",        sngl(vars%fdy),  dims=[dim_lon,dim_latv,dim_lev,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="meridional diffusive tracer volume flux",units="m3*K",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"fdz",        sngl(vars%fdz),  dims=[dim_lon,dim_lat,dim_levw,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="vertical diffusive tracer volume flux",units="m3*K",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"drho_dx",      sngl(vars%drho_dx),dims=[dim_lon,dim_lat,dim_lev,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="x density gradient",units="kg/m4",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"drho_dy",      sngl(vars%drho_dy),dims=[dim_lon,dim_lat,dim_lev,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="y density gradient",units="kg/m4",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"drho_dz",      sngl(vars%drho_dz),dims=[dim_lon,dim_lat,dim_lev,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="z density gradient",units="kg/m4",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"Ri",           sngl(vars%Ri),dims=[dim_lon,dim_lat,dim_lev,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="Richardson number",units="1",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"slope_x",      sngl(abs(vars%drho_dx/min(vars%drho_dz,-1.e-6))),dims=[dim_lon,dim_lat,dim_lev,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="x isopycnal slope",units="m/m",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"slope_y",      sngl(abs(vars%drho_dy/min(vars%drho_dz,-1.e-6))),dims=[dim_lon,dim_lat,dim_lev,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="y isopycnal slope",units="m/m",missing_value=missing_value,ncid=ncid)
    endif
    if (l_output_extended .or. l_diff_dia_strat) then
      call nc_write(fnm,"diffdia",      sngl(vars%diffdia),dims=[dim_lon,dim_lat,dim_lev,dim_month,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="diapycnal diffusivity",units="m2/s",missing_value=missing_value,ncid=ncid)
    endif
  ...
  subroutine ocn_daily_nc(fnm)
    ...
    call nc_write_dim(fnm,dim_lon,x=lon,axis="x",ncid=ncid)
    call nc_write_dim(fnm,dim_lat,x=lat,axis="y",ncid=ncid)
    call nc_write_dim(fnm,dim_lev,x=zro,units="m",axis="z",ncid=ncid)
  ...
  subroutine ocn_daily_nc_write(fnm,ncid,vars,ndat,nout)
    ...
    call nc_write(fnm,"t",        sngl(vars%t),  dims=[dim_lon,dim_lat,dim_lev,dim_day,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="temperature",units="C",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"s",        sngl(vars%s),  dims=[dim_lon,dim_lat,dim_lev,dim_day,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="salinity",units="psu",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"rho",      sngl(vars%rho),  dims=[dim_lon,dim_lat,dim_lev,dim_day,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="density",units="kg/m3",missing_value=missing_value,ncid=ncid)
    if (cons_tracer) call nc_write(fnm,"cons",        sngl(vars%cons),  dims=[dim_lon,dim_lat,dim_lev,dim_day,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="conservative tracer",units="1",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"u",        sngl(vars%u),  dims=[dim_lon,dim_lat,dim_lev,dim_day,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="zonal velocity",units="m/s",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"v",        sngl(vars%v),  dims=[dim_lon,dim_lat,dim_lev,dim_day,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="meridional velocity",units="m/s",missing_value=missing_value,ncid=ncid)
    call nc_write(fnm,"w",        sngl(vars%w),  dims=[dim_lon,dim_lat,dim_lev,dim_day,dim_time],start=[1,1,1,ndat,nout],count=[maxi,maxj,maxk,1,1],long_name="vertical velocity",units="m/s",missing_value=missing_value,ncid=ncid)
