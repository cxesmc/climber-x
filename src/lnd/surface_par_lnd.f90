...
contains
  ...
  subroutine resist_aer(frac_surf,veg_h,lai,sai,h_snow,tatm,t_skin,wind, &
    ...
    ! The wind-only, wind+leaf-area and plain r_a(i_bare) variants (i_racan 1, 2 and 3) were
    ! removed along with the i_racan switch and p_cdense.
    ! NOTE r_a(i_bare) alone cannot represent the decoupling of the ground below a closed
    ! canopy: r_a depends only logarithmically on roughness, so bare soil and forest differ
    ! by less than a factor 2. c_racan_1 sets the amplitude of the decoupling.
    ! c_racan_2 sets how fast the closed-canopy limit is approached with leaf area. With
    ! c_racan_2=1 the exponential saturates by lai+sai~3 (95% of the maximum), so a dense
    ! tropical canopy and a semi-arid shrubland are given almost the same below-canopy
    ! resistance; the zonal soil-evaporation profile then lacks the equatorial minimum the
    ! CMIP6 models show. c_racan_2<1 stretches the curve over the observed leaf area range
    ! and restores the contrast (at c_racan_2=0.1: lai+sai=1 -> 0.10, =6 -> 0.45).
    ! NOTE sai is time-invariant and non-zero, so the open canopy limit is only reached where
    ! sai is small; for trees it leaves a floor of c_racan_1*(1-exp(-c_racan_2*sai))*r_a(i_bare).
    do n=1,npft
  ...
  ! ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  subroutine resist_sur(frac_surf,mask_snow,theta_w, g_can, &
                        beta_s,r_s,beta_s_can,r_s_can)
    ...
    integer :: n
    real(wp) :: f_snow, beta_soil, beta_snow, r_soil
