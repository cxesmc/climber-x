module smb_surface_par_m
    ! Surface parameters (albedo, aerodynamic resistance) for the SMB
    ! single-column physics.
    !
    ! Ported into the lndvc framework from src/smb/smb_surface_par.f90 (module
    ! smb_surface_par) with the bodies unchanged; parameter/constant
    ! dependencies re-homed onto the framework-owned modules so lndvc is
    ! self-contained.

    use precision, only : wp
    use const_m, only : karman, g, pi, T0, rho_i, z_sfl
    use smb_par_m, only : dt, l_neutral
    use smb_par_m, only : snow_par, surf_par
