module smb_temp_m
    ! Soil/ice temperature for the SMB single-column physics.
    !
    ! Ported into the lndvc framework from src/smb/smb_temp.f90 (module
    ! smb_temp_mod) with the body unchanged; parameter/constant/grid
    ! dependencies re-homed onto the framework-owned modules (const_m, smb_par_m,
    ! smb_grid_m) so lndvc is self-contained. tridiag is a shared numerical util.

    use precision, only : wp
    use const_m, only : Lf, T0, cap_i, lambda_i, rho_i
    use smb_par_m, only : dt, rdt, snow_par, check_energy
    use smb_grid_m, only : z, z_int, dz, rdz_pos, rdz_neg, nl
