module smb_downscaling_m
    ! Downscaling of climate fields to a virtual cell's elevation and slope.
    !
    ! Ported into the lndvc framework from src/smb/downscaling.f90 (module
    ! downscaling_mod) with the routine bodies unchanged; only the parameter/
    ! constant dependencies are re-homed onto the framework-owned const_m and
    ! smb_par_m so lndvc is self-contained. These are the shared single-column
    ! downscaling kernels used by every surface class.

    use precision, only : wp
    use const_m,   only : pi, T0, frac_vu
    use smb_par_m, only : prc_par, wind_ele_fac, dLW_dT_fac, d_cld
