module semi_m
    ! Main SEMIX single-column surface energy and mass balance driver.
    !
    ! Ported into the lndvc framework from src/smb/semi.f90 (module semi_m) with
    ! the body unchanged; module dependencies re-homed onto the framework-owned
    ! ports (thermo_m, smb_par_m, smb_downscaling_m, smb_surface_par_m,
    ! smb_ebal_m, smb_temp_m, smb_snow_m) so lndvc is self-contained.

    use precision, only : wp
    use thermo_m, only : fqsat, q_sat_i
    use smb_par_m, only : dt, i_gamma, gamma, l_regional_climate_forcing, prc_par, snow_par
    use smb_downscaling_m, only : wind_downscaling, prc_downscaling, rad_downscaling
    use smb_surface_par_m, only : surface_albedo, resistance
    use smb_ebal_m, only : ebal, update_tskin
    use smb_temp_m, only : smb_temp
