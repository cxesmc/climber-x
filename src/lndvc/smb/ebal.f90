module smb_ebal_m
    ! Surface energy balance for the SMB single-column physics.
    !
    ! Ported into the lndvc framework from src/smb/smb_ebal.f90 (module
    ! smb_ebal_mod) with the bodies unchanged; parameter/constant/grid/thermo
    ! dependencies re-homed onto the framework-owned modules so lndvc is
    ! self-contained.

    use precision, only : wp
    use const_m, only : Ls, cap_a, sigma, lambda_i, T0, pi
    use thermo_m, only : q_sat_i, dqsat_dT_i, rho_a
    use smb_par_m, only : surf_par, snow_par, l_diurnal_cycle, tstd_scale, dt, l_dew, check_energy
    use smb_grid_m, only : z, nl
