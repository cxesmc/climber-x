...
module lndvc_soil_hydro_mod
  ...
  use lnd_grid, only : nsurf, nveg, npft, nsoil, flag_veg, flag_pft
...
contains
  ...
  subroutine soil_hydro(frac_surf,mask_snow,theta_sat,k_sat,k_exp,psi_exp,kappa_int,psi,w_table, &
    ...
    if( hydro_par%i_runoff .eq. 1 ) then
     ...
     drainage = kappa_int(nl) + dk_dtheta(nl)*x(nl)
    elseif( hydro_par%i_runoff .eq. 2 ) then
     drainage = run_sub
    endif
    ...
    if (l_wiso) then
      ...
      if (hydro_par%i_runoff .eq. 1) then
        drainage_iso(:) = q_iso(nl,:)
      else
        do iso=1,nwiso
          drainage_iso(iso) = sum(r_sub_iso(:,iso))
        enddo
      endif
    endif
    ...
    do k=1,nl-1 ! start from top layer
     if(w_w(k) .gt. w_w_max(k)) then
      ...
      if (l_wiso .and. w_w(k).gt.0._wp) then
        do iso=1,nwiso
          ...
          if (k.eq.1) then
            ...
            drainage_iso(iso) = drainage_iso(iso) + 0.5_wp*excess_iso(iso) * rdt
          else
      ...
      if(k.eq.1) then
       ...
       drainage = drainage + 0.5_wp*(w_w(k) - w_w_max(k)) * rdt
      else
    ...
    if(w_w(nl) .gt. w_w_max(nl)) then ! bottom layer oversaturated
     if (l_wiso .and. w_w(nl).gt.0._wp) then
       do iso=1,nwiso
         ...
         drainage_iso(iso) = drainage_iso(iso) + ratio(iso)*(w_w(nl) - w_w_max(nl)) * rdt
...
lp1:  do k=nl-1,1,-1 ! search for the required amount of water in layers above
      ...
      if(i_here .eq. 0) then ! not enough water in the layers, remove additional necessary water from drainage
       if (l_wiso) then
         do iso=1,nwiso
           ...
           drainage_iso(iso) = drainage_iso(iso) - (w_w_min - w_w(nl) - dw) * Rstd(iso) * rdt
           w_w_iso(:,iso) = w_w_min * Rstd(iso)
         enddo
       endif
       drainage  = drainage - (w_w_min - w_w(nl) - dw) * rdt
       w_w = w_w_min ! set w_w to w_w_min in all layers
      endif

      if(check_water .and. drainage*dt .lt. -0.1_wp) then
       print *,' '
       print *,' '
       print *,'WARNING, negative drainage!! ',drainage*dt,' mm/day'
    ...
    if( check_water ) then
     ! water balance
     water_bal =  sum(inf)*dt - sum(e)*dt - drainage*dt &
               - sum(w_w-ww_old)
...
end module lndvc_soil_hydro_mod
