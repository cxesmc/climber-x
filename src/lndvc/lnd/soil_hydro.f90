    ...
    if(w_w(nl) .gt. w_w_max(nl)) then
     ...
     if (l_wiso .and. w_w(nl).gt.0._wp) then
       do iso=1,nwiso
         ...
         drainage_iso(iso) = drainage_iso(iso) + dw_iso * rdt
         w_w_iso(nl,iso) = w_w_iso(nl,iso) - dw_iso
       enddo
     endif
     drainage = drainage + excess * rdt ! add excess to drainage, kg/m2/s
     w_w(nl) = w_w_max(nl)
    endif
...
lp1:  do k=nl-1,1,-1 ! search for the required amount of water in layers above
    ...
    if( check_water ) then
     ! water balance
     water_bal =  sum(inf)*dt - sum(e)*dt - drainage*dt - runoff_exc*dt &
               - sum(w_w-ww_old)
