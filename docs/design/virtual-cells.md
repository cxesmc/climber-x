...
**Later:** cluster-validate the identity ice + lake paths; wire the deferred sublake column (with soil-param sourcing) alongside lake carbon; port the veg path (photosynthesis/phenology/soil-carbon/dyn_veg — the large one, reusing the B.1 single-tile surface routines, generalized to veg/ice); Phase 3 multiple land bands (populate the currently-zeroed `z_sur_std`/`dz_sur`/slopes/`f_ele`).
