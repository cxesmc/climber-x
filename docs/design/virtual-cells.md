...
**Next step — (b) wire the lake single-column path (`lndvc_update_lake`).** Confirmed design: lake uses the land model's forcing directly (no SEMI/`_i` downscaling); at identity (n_vc=1, cell elevation) forcing is a straight copy, per-elevation lake downscaling deferred. Scoping finding: the lake **thermal chain is already single-column** (`ebal_lake`/`update_tskin_lake`/`lake_par_thermal`/`sublake_par_thermal`/`lake_temp`/`lake_convection`/`sublake_temp` are called per-tile with scalar args in the reference) — no refactor. Only 5 routines still loop over tiles. Plan (each compile-checkable; **refactor each against the `src/lnd` original by eye — no cluster validation available**):
1. **B.1** — refactor `snow_albedo` / `surface_albedo` / `resist_aer` / `resist_sur` (`src/lndvc/lnd/surface_par.f90`) to single-column single-tile signatures (helpers already single-column; `snow_albedo`'s three veg/ice/lake branches collapse to one). Shared by lake + future veg/ice.
2. **B.2** — refactor `surface_hydrology` to a single lake tile (the delicate one: it has cross-tile `nsurf` sums that collapse for one tile).
3. **B.3** — `alloc_lake_blocks` in `set_leaf` (mirror `alloc_ice_blocks`: size `lake_col`/`sublake` thermal columns `nl_l`/`nl` + snow + lake-tile `flx`, init once); extend `cmn_to_lndvc` to fill lake vcs' *downscaled* `forcing_t` slots (`tatm/qatm/wind/swnet/lwdown/rain/snow/pressure`) from the coarse cell.
4. **B.4** — wire `lndvc_update_lake` (unpack lake blocks → single-column chain) + extend `aggregate_cell` for lake surface fluxes.

**Later:** cluster-validate the identity ice + lake paths; port the veg path (photosynthesis/phenology/soil-carbon/dyn_veg — the large one, reusing the B.1 single-column surface routines); Phase 3 multiple land bands (populate the currently-zeroed `z_sur_std`/`dz_sur`/slopes/`f_ele`).
