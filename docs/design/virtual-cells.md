...
- **Next actions**: (1) **lake carbon + sublake column follow-up** (the agreed C.4 companion PR: wire the already-ported `sublake_par_thermal`/`sublake_temp` + a sublake soil block + `lake_carbon`/`lake_carbon_par` on the lake vc); (2) the **cell-level carbon wrapper** (carbon + emission aggregation + `carbon_trans`/`weathering`/`carbon_export`/`carbon_flux_atm_lnd` + global CH₄/CO₂/N₂O accounting); (3) cluster-validate the `n_vc=1` identity paths. Confirm sub-commit shape with the user before each.
