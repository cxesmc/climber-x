...
- **Next actions**: (1) the **cell-level carbon wrapper** (carbon + emission aggregation + `carbon_trans`/`weathering`/`carbon_export`/`carbon_flux_atm_lnd` + global CH₄/CO₂/N₂O accounting); (2) cluster-validate the `n_vc=1` identity paths. Confirm sub-commit shape with the user before each.
