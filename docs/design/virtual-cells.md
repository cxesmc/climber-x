...
**Status:** architecture (decisions 1–3) confirmed. **C.1 (surface energy balance) and C.2 (soil hydrology) done** — compile+link only under `LNDVC=1`, default green; validation cluster-gated (local geo-init segfault).
...
- **Next actions**: (1) **lake carbon + sublake column follow-up** (the agreed C.4 companion PR: wire the already-ported `sublake_par_thermal`/`sublake_temp` + a sublake soil block + `lake_carbon`/`lake_carbon_par` on the lake vc); (2) **C.5 emissions** (`dust_emission`, `n2o_emission`); (3) the **cell-level carbon wrapper** (carbon aggregation + `carbon_trans`/`weathering`/`carbon_export`/`carbon_flux_atm_lnd`). Confirm sub-commit shape with the user before each.
