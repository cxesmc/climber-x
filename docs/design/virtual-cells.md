...
- **Makefile dependency gap:** `obj/coupler*.o` and the `src/main` objects don't declare a dependency on `lndvc_def.mod`, so a change to `lndvc_def.f90` that doesn't also touch the coupler needs the stale objects removed by hand (`rm obj/coupler_nobgc.o obj/climber_clim.o`) before `make climber-clim`, else the build fails with a `vc_cell_t` component mismatch.
