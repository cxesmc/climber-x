...
- **Next action is C.4** (soil carbon): `soil_carbon_par` → `soil_carbon` → `peat_carbon`; then the deferred sublake column + lake carbon. C.4 also supplies the physical carbon cold-start that C.3 left zero-initialised.
