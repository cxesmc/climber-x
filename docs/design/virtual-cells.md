...
- **Lake port A — physics bodies ported** (`src/lndvc/lnd/`, pattern A, modules renamed `lndvc_*`, distinct `lndvc_lnd_*.o` objects; compiles + links under `LNDVC=1`, not yet wired). Files: `surface_par.f90` (`snow_albedo`, `surface_albedo`, `resist_aer`, `resist_sur`, + single-column helpers), `lake_par.f90` (`lake_par_thermal`, `sublake_par_thermal`), `ebal_lake.f90` (`ebal_lake`, `update_tskin_lake`), `lake_temp.f90`, `lake_convection.f90`, `sublake_temp.f90`, `surface_hydro.f90` (`surface_hydrology`, `canopy_water`). **Decision (A): reuse the real land infrastructure** (`lnd_grid`/`lnd_params`/`wiso_params`/`lake_rho`) rather than duplicating it — one source of truth; a later `use` swap makes `lndvc` standalone. Only physics *bodies* are copied.

- **(b) Lake single-column path is live** (compile + link only; run validation gated on the cluster). Lake uses the land model's forcing directly (no SEMI/`_i` downscaling); at identity (`n_vc=1`, cell elevation) forcing is a straight copy, per-elevation lake downscaling deferred. Scoping finding: the lake **thermal chain was already single-column** (`ebal_lake`/`update_tskin_lake`/`lake_par_thermal`/`lake_temp`/`lake_convection`/`sublake_temp` are per-tile scalar calls); only the 4 surface routines + `surface_hydrology` looped over tiles. Four commits (`b85ca62`, `beaa9d4`, `8dac470`, `4ede3f3`):
  1. **B.1** — `snow_albedo`/`surface_albedo`/`resist_aer`/`resist_sur` (`src/lndvc/lnd/surface_par.f90`) → lake-only single-tile `*_lake` (scalar in/out). **Decision (lake-only, not class-generic):** the tile-local albedo routines generalize to veg/ice later for free, while the resistances' bare→canopy cross-tile coupling (`r_a_can=r_a(i_bare)`, `r_s_can=r_s(i_bare)`) can't be resolved single-tile until the veg path exists — so B keeps them lake-scoped. Multi-tile bodies dropped (live in `src/lnd/surface_par_lnd.f90`).
  2. **B.2** — `surface_hydrology` → `surface_hydrology_lake`. The lake block is self-contained (the cross-tile `nsurf`/`f_veg` sums are veg-only), so it de-loops directly. `canopy_water` untouched (veg, later).
  3. **B.3** — `alloc_lake_blocks` in `set_leaf` (lake_col `0:nl_l` + sublake `nl` thermal columns + iso, snowpack + iso, **full** `surface_flux_t` via new `alloc_surface_flux` helper; bounds mirror `src/lnd/lnd_model.f90`; **lean init `T0`/0** — physical `init_cell_lake` spin-up deferred). `forcing_t` gained a plain `dust` slot; `vc_desc_t` gained `lat`. `cmn_to_lndvc` fills lake vcs' downscaled `forcing_t` slots (straight copy of the coarse-cell lake tile, `i_surf_lake=9`) + per-vc `h_lake` + `lat` (mirror `cmn_to_lnd`).
  4. **B.4** — wire `lndvc_update_lake` (pattern-A unpack → resist_aer/snow_albedo/surface_albedo/resist_sur → lake_par_thermal → ebal_lake → lake_temp → update_tskin_lake → surface_hydrology_lake; no-canopy throughfall, `w_snow_old` snapshot at entry, `t_skin_old` done inside `ebal_lake`) + extend `aggregate_cell` for lake surface fluxes (t_skin/albedo/flx_sh/flx_lh/flx_g/evap/et intensive mean; `lake_water_tendency` as conserving runoff sum). **Deferred:** `sublake_par_thermal`/`sublake_temp` (need soil params not on the lake block; feed only downstream sublake-soil / lake-carbon state, not the surface coupling currency) and the melt-iso redistribution from the reference `lnd_model` wrapper (like the SMB-iso deferral).

**Later:** cluster-validate the identity ice + lake paths; **port C = the veg path** (roadmap below) — DONE; the deferred sublake column + lake carbon — DONE (see the lake-carbon follow-up below); Phase 3 multiple land bands (populate the currently-zeroed `z_sur_std`/`dz_sur`/slopes/`f_ele`).
...
3. **Whole-cell operations stay at cell level** — `carbon_trans` (cross-class carbon on fraction changes), `weathering`, and `carbon_export` are inherently all-tile; they run in a cell-level wrapper *after* vc aggregation, reading the aggregated vc carbon pools. Not ported into the single-vc physics.

**Staging (each a separate PR, own sub-commits):**
- **C.1 — vegetated surface energy balance** (the coupling-currency slice): `nveg` surface routines + soil-param plumbing (decision 2) → `phenology` + `photosynthesis` (→ `g_can`) → `resist_aer`/`resist_sur` (+ `canopy_water`) → `snow_albedo`/`surface_albedo` → `soil_par_thermal` → `ebal_veg` → `soil_temp` → `update_tskin_veg`; then wire `lndvc_update_land` + extend `aggregate_cell` for veg fluxes.
- **C.2 — soil hydrology:** `soil_par_hydro` → `surface_hydrology` (refactor its internal `f_veg = sum(frac_surf,…)` to the vc's own tiles) → `soil_hydro` → `calculate_pet`/`calculate_cwd`.
...
**Status:** architecture (decisions 1–3) confirmed. **Port C COMPLETE (C.1 surface energy balance → C.2 soil hydrology → C.3 vegetation dynamics → C.4 soil carbon → C.5 emissions)** — compile+link only under `LNDVC=1`, default green; validation cluster-gated (local geo-init segfault).
- **C.2 COMPLETE** (5 sub-commits): **C.2a** `surface_hydrology_veg` (veg-scoped scalarisation of the all-class routine; ice branch dropped → SEMI owns it, lake branch stays `surface_hydrology_lake`); **C.2b** pattern-A `soil_hydro` (drainage/`drainage_iso` scalarised to the single veg column) + `calculate_pet`/`calculate_cwd` (pure cp); **C.2c** data model (`cwd_mon` + static TOPMODEL/DYPTOP `cti_mean`/`cti_cdf`/`dyptop_*` on `soil_col_t`, seeded in `lndvc_init_land` from the global `topmodel(i,j)`/`dyptop(i,j)`); **C.2d** wire the chain into `lndvc_update_land` (surface_hydrology_veg → soil_par_hydro → soil_hydro → calculate_pet → calculate_cwd → runoff = runoff_sur + drainage); **C.2e** `aggregate_cell` adds veg runoff as a conserving cell sum. **Deferred:** calving / `f_wet` / `w_table` / `mcwd` have no cell coupling slot yet; the soil-temp/hydro iso freeze-thaw redistribution stays deferred (as in C.1).
- **C.3 COMPLETE** (vegetation dynamics; 3 sub-commits, compile+link only, default green). Physics were mostly already present (`dynveg_par`/`root_frac_update`/`litter_in_frac_update` in lndvc `veg_par.f90`, `surface_frac_up` in lndvc `surface_par.f90`); only `dyn_veg` needed porting.
  - **C.3a**: pattern-A cp `dyn_veg.f90` (self-contained, no internal physics calls).
  - **C.3b**: cross-class fraction plumbing. lndvc runs *alongside* the reference lnd (`lnd_update` always runs first), so `cmn_to_lndvc` gains a `lnd` arg and copies the already-computed `lnd%l2d(i,j)` fractions (`f_{veg,ice_grd,lake,shelf}` +`_old`, `f_ice_nbr`) into `forcing_t` — decision A, exact identity, no duplicated geography/neighbor logic. f_crop/f_pasture/disturbance also copied; `veg_t` gains `dyn_veg`'s fire-flux + carbon-balance outputs.
  - **C.3c**: wire three time-gated blocks into `lndvc_update_land` — `time_soy_lnd` (`set_frac_surf_veg` [new helper, within-vc pft→frac_surf per decision 2; `init_land` refactored onto it] → `soil_par_update` → `root_frac_update` → `litter_in_frac_update`), `time_eom_lnd` (`dynveg_par`), `time_call_veg` (reused from `lnd_params`; `dyn_veg` → frac resync). Mineral-soil baseline persisted on `soil_col_t` for `soil_par_update`.
  - **Decision 3 honored**: class transitions / `carbon_trans` stay cell-level, deferred (no transitions at n_vc=1 static geo). No separate aggregate commit — C.1e veg aggregation already consumes the updated `frac_surf`; fire-CO2 flux is a C.5 currency.
  - **Option A (lean carbon init)**: `dyn_veg`/`soil_par_update`/`dynveg_par` read soil-carbon pools (`litter_c`/`fast_c`/`slow_c`/`cato_c`/`frac_soc`) and `dyn_veg` writes `litterfall(nlc,ncarb)`; these are allocated + **zero-init** in `lndvc_decompose`. **Physical carbon cold-start + veg-carbon identity deferred to C.4.** Also fixed two data-model shapes: `disturbance` scalar→`veg_t(npft)`, `litterfall*` `(:)`→`(:,:)`.
- **C.4 (land soil carbon) COMPLETE** (3 sub-commits, compile+link only, default green). All three routines are single-column pattern-A ports.
  - **C.4a**: pattern-A cp `soil_carbon_par`/`soil_carbon`/`peat_carbon` → lndvc.
  - **C.4b**: complete `soil_carbon_t` allocation in `lndvc_decompose` (all mineral+peat pools incl. ¹³C/¹⁴C, `k_*` decomposition rates, `ftemp`/`fmoist`/`fdepth`, per-carbon-class totals/respiration, conservation residuals), all zero-init; physical cold-start (peat `f_peat = f_peat_min·f_veg`) already done by `lndvc_init_cell_veg`, closing the C.3 Option-A deferral. Data-model fixes: `carbon_cons_soil/13/14` scalar→`(ncarb)`; `f_wet_mon`/`w_table_mon` scalar→`(nmon_year)`, `f_wet_long`→`(nmonwet)`.
  - **C.4c**: wire into `lndvc_update_land` in reference order — `soil_carbon_par` (`time_call_carb_p`, before the C.3 eom/dyn_veg blocks) → `dyn_veg` → `soil_carbon`+`peat_carbon` (`time_call_carb`, guarded `f_veg>0` / `peat_par%peat_carb`). Timing flags reused from `lnd_params`; `dust_dep`←`forc%dust`.
  - **No aggregate step (decided Option A)**: soil-carbon coupling is inherently cell/global-level (CH₄→global 0-D `l0d%ch4_emis`; CO₂→`carbon_flux_atm_lnd`, the deferred cell wrapper). `vc_cell_t` has no consumer for aggregated carbon and the per-vc pools already feed `soil_par_update` locally, so carbon aggregation is folded into the deferred **cell-level carbon wrapper** stage (with `carbon_trans`/`weathering`/`carbon_export`, decision 2). `ice_carbon`/`shelf_carbon` out of scope.
- **C.5 (emissions) COMPLETE** (3 sub-commits, compile+link only, default green). Both routines are single-column pattern-A ports; output slots (`carb%n2o_emis`, `carb%dust_emis_*`) already existed in the data model.
  - **C.5a**: pattern-A cp `dust_emis`/`n2o_emis` → lndvc (new `lndvc_lnd_{dust,n2o}_emis.o`); self-contained, reuse real `lnd_params` (`dust_par`/`n2o_par`), `lnd_grid` indices, `constants` `T0`.
  - **C.5b**: wire `n2o_emission` inside `time_call_carb` after `peat_carbon`, guarded `f_veg>0` (reference order); combined `soil_resp = soil_resp(ic_min)·(f_veg−f_peat) + soil_resp(ic_peat)·f_peat`. No new plumbing.
  - **C.5c**: plumb cell topography `z_veg`/`z_veg_min`/`z_veg_max` into `forcing_t`, copied from `lnd%l2d` in `cmn_to_lndvc` (decision A; the reference values already carry the neighbor min/max expansion). Wire `dust_emission` after the carbon block, every land step, guarded `f_veg>0`: `z_veg_std`←`desc%z_sur_std` (matching `dyn_veg`, = 0 at identity — the shared Phase-3 deferral), `tatm` the sub-tile broadcast, `wind` the bare-tile scalar → `carb%dust_emis_*`.
  - **No aggregate step (Option A, as C.4)**: `n2o_emis`/`dust_emis` are per-vc in the `carb` block; aggregation to the cell (and `vc_cell_t%n2o_emis`; no `dust_emis` cell slot yet) is deferred to the cell-level wrapper. `weathering`/`carbon_export` stay cell-level.
- **Port C COMPLETE** (C.1 SEB → C.2 hydrology → C.3 veg dynamics → C.4 soil carbon → C.5 emissions; all compile+link only, `LNDVC=1` + default green; validation cluster-gated).
- **Lake carbon + sublake follow-up COMPLETE** (the agreed C.4 companion; 3 sub-commits L.1–L.3, compile+link only, `LNDVC=1` + default green). Closes the port-B sublake deferral and adds lake sediment carbon on the lake vc. **User decision: everything lake-specific lives on `lake_col_t`** (not a separate `soil_col`/`carb` block on the lake vc). **Key finding: `dyn_veg` zeroes the whole `litterfall` array and only writes the `ic_min` column, so `litterfall(:,ic_lake)` is always zero; `carbon_trans` only redistributes *pools* on fraction changes (a no-op at static `n_vc=1` geo). So lake carbon is self-contained on the lake vc at identity** — no cell wrapper needed to reproduce.
  - **L.1** (sublake thermal): `lake_col_t` gains the sublake mineral soil params (`theta_sat`/`lambda_s`/`psi_sat`/`psi_exp`, `nl`) + per-layer lake-carbon heat `soil_resp_l` (`nl,ncarb`), allocated + zero/T0-init in `alloc_lake_blocks`. `cmn_to_lndvc` class==2 seeds the soil params from `lnd%l2d` (decision A — the reference cell params the sublake reads, computed this step before `lndvc_update`). Wire `sublake_par_thermal` + `sublake_temp` after `lake_temp` (`t_sublake(0)=t_lake(nl_l)`).
  - **L.2** (lake-carbon data model): pattern-A cp `lake_carbon`/`lake_carbon_par` → lndvc (new `lndvc_lnd_lake_carbon{,_par}.o`). `lake_col_t` gains sediment pools (`litter/fast/slow_c_lake` +¹³/¹⁴C, `nlc`), rates (`k_*_lake`/`diff_lakec`/`adv_lakec`, `nlc`), `ch4_frac_lake` (`nl`), and the `ic_lake` scalar respiration/totals/`ch4_emis_lake`/conservation; allocated + cold-start zero.
  - **L.3** (wire lake carbon): `lake_carbon_par` (`time_call_carb_p`, from cumulative sublake T/moisture) + `lake_carbon` (`time_call_carb`) at the end of `lndvc_update_lake`. `litterfall(ic_lake)=0` (local zero array); no `f_lake` guard (the lake vc's existence is the gate, as the surface chain); `soil_resp_l(:,ic_lake)` feeds back into next step's `sublake_temp`. **No aggregate step (Option A)**: `ch4_emis_lake` per-vc, deferred to the cell wrapper.
  - **Still deferred on the lake vc**: physical lake spin-up (`init_cell_lake`: `t_lake` ramp, sublake `theta` from soil, cross-tile skin mean) — the lake vc keeps lean T0/0 init; and the melt-iso redistribution (as SMB iso).
- **Cell-level carbon wrapper COMPLETE** (port W, compile+link only, `LNDVC=1` + default green). Reconnaissance corrected decision 3's grouping: in the reference, `weathering`/`carbon_export` run *inside* `lnd_update` (per-column, eoy, reading only veg/peat state) — they are **land-vc** ops, not cross-class; only `carbon_trans` is genuinely cross-class. So the wrapper split into a land-vc slice (W.4) + genuine post-loop aggregation/accounting (W.1/W.2/W.5).
  - **W.1** (`7e74f8e`): emission aggregation — `lndvc_aggregate_cell` reduces per-vc CH₄/N₂O to `vc_cell_t%ch4_emis`/`n2o_emis` (land vc wetland+peat CH₄, N₂O; lake vc CH₄; shelf CH₄ out of scope); `lndvc_update` accumulates the annual global `glob%ch4_emis`/`n2o_emis` (kgC, `flux·dt·area`), reset at soy.
  - **W.2** (`adc56a1`): `carbon_flux_atm_lnd` — `vc_cell_t` gains `f_peat` (from the land vc); at eom, assemble the cell `soil_resp(ncarb)`/`npp_real`/`fire_c_flux` from the vcs (land ic_min/ic_peat, lake ic_lake, ic_ice/ic_shelf = 0) → `cell%Cflx_atm_lnd` + annual `glob%Cflx_atm_lnd`.
  - **W.4** (`fefe751`+`72ad39f`): `weathering` + `carbon_export` on the **land vc** (reference order, eoy). Fixed the carb lithology stubs scalar→`(nlit_*)` arrays; plumbed the `lit_map(i,j,:)` slice + `weath_scale` via `cmn_to_lndvc` (decision A). Outputs on the carb block.
  - **W.5** (`d4fdb58`): `carbon_inventory` + burial + running weathering averages (post-loop, eoy). Helper `lndvc_carbon_inventory_cell` assembles the cell pools from the vcs (veg+mineral+peat from land vc, lake sediment from lake vc; ice/shelf zero); total land carbon → `glob%landc`, burial flux (`l_burial` branch or return-to-atmosphere with the per-cell Cflx adjustment), `Cflx_avg` + global carbonate/silicate weathering averages.
  - **`carbon_trans` DEFERRED** (`09b4344` note): the only genuinely cross-class op; a no-op at static `n_vc=1` geo and needs cell→vc pool write-back machinery — deferred to the dynamic-fraction phase (Phase 3).
  - **Option A gap (user-confirmed)**: `ice_carbon`/`shelf_carbon` out of scope, so `soil_resp(ic_ice/ic_shelf)`, shelf CH₄, and the ice/shelf term of total land carbon are zero. Bites only cells with grounded-ice / submarine-shelf sediment carbon; a later **port D** would close it.
  - **Latent bug fixed** (`09b4344`): C.4b allocated the mineral soil-carbon pools + decomposition rates `(nl)` where the pattern-A `soil_carbon`/`soil_carbon_par`/`soil_par_update`/`carbon_export`/`carbon_inventory` routines index `1:nlc` (`nlc=nl+1`, burial layer). Latent (land path compile-only). Resized 22 fields to `nlc` (`litter/fast/slow_c` ±¹³/¹⁴C, `cato_c` ±¹³/¹⁴C, `k_*`, `k_slow_to_fast`, `k_cato`, `diff/adv_soilc`); `frac_soc`/`ch4_frac_*`/`ftemp`/`fmoist`/`fdepth`/`litter_in_frac`/`soil_resp_l` stay `nl` (verified). L.2 lake pools were already `nlc`.
- **Next actions**: (1) **cluster-validate** the `n_vc=1` identity paths (ice+lake+veg must match the reference land) — still blocked locally by the geo-init segfault; (2) optional **port D** (`ice_carbon`/`shelf_carbon`) to close the Option-A gap; (3) `carbon_trans` when dynamic fractions arrive (Phase 3). Confirm sub-commit shape with the user before each.
